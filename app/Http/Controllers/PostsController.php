...
class PostsController extends Controller
{
    ...
    {
        $post=Post::with('usuario','categoria', 'comentarios', 'comentarios.usuario')->find($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->file('imagen')){
            ...
            if($post->imagen != 'default.png'){
                if(file_exists(public_path().'/imagenes/posts/'.$post->imagen)){
                    unlink(public_path().'/imagenes/posts/'.$posts->imagen);
                }
            }
        ...
        $post->usuario_id = auth()->user()->id;
        if($post->save()){
            return redirect('/posts')->with('success','Registro actualizar correctamente');
        }else{
